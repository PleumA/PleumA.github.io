...
- **Person-Centric View**: Added a toggle to pivot the table from a date-centric grid (columns = days) to a person-centric grid (rows = doctors, columns = highlighted shift days). *Requires writing a new `renderPersonCentricView` function and a dedicated tab button.*
